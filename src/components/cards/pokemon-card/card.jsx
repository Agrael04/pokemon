...
export default ({ pokemon, showPokemon, removePokemon }) => (
  <div className={styles.card} onClick={showPokemon}>
    ...
    <div className={styles.stats}>
      {pokemon.stats.map((stat, key) => <div key={'stat' + key}>{stat.stat.name}: {stat.base_stat}</div>)}
    </div>
    <div className={styles.abilities}>
      {pokemon.abilities.map((ability, key) => <div key={'abil' + key}>{ability.ability.name}</div>)}
    </div>
  </div>
) 